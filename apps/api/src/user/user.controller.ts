...
export class UserController {
  ...
  @ApiQuery({ name: 'sortBy', required: false, enum: ['createdAt', 'updatedAt', 'name', 'email'], description: 'Sort field' })
  ...
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search by name or email' })
  async getUserCount(@Query('search') search?: string): Promise<{ count: number }> {
    let where;
    if (search) {
      where = {
        OR: [
  ...
  @ApiResponse({ status: 200, description: 'User with social accounts retrieved successfully', type: UserEntity })
  @ApiResponse({ status: 404, description: 'User not found' })
  @ApiParam({ name: 'id', type: 'string', description: 'User ID' })
  ...
  @ApiResponse({ status: 200, description: 'User with recent posts retrieved successfully', type: UserEntity })
  @ApiResponse({ status: 404, description: 'User not found' })
  @ApiParam({ name: 'id', type: 'string', description: 'User ID' })
  @ApiQuery({ 
    name: 'limit', 
